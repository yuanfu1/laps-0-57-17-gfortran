...
c
c     l is a real array of dimension n.
c       On entry l is the lower bound on x.
c       On exit l is unchanged.
c
c     u is a real array of dimension n.
...
c
c     wa is a real working array of length 
...
c
c     dsave is a real working array of dimension 29.
c       On exit with 'task' = NEW_X, the following information is
c                                                             available:
...
c       On exit pgtol is unchanged.
c
c     ws, wy, sy, and wt are real working arrays used to
...
c                                  (2.26) in [3].
c
c     wn is a real working array of dimension 2m x 2m
...
c                     [ 0  I]
c
c     snd is a real working array of dimension 2m x 2m
c       used to store the lower triangular part of
c                 N = [Y' ZZ'Y   L_a'+R_z']
c                     [L_a +R_z  S'AA'S   ]
c	     
c     z(n),r(n),d(n),t(n),wa(8*m) are real working arrays.
c       z is used at different times to store the Cauchy point and
c       the Newton point.
c
c     sg(m),sgo(m),yg(m),ygo(m) are real working arrays. 
c
...
c     ************
     ...
     +                 iword,nfree,nact,ileave,nenter
      real theta,fold,ddot,dr,rr,tol,dpmeps,
     ...
     +                 gd,gdold,stp,stpmx,time
      ...
      ! Yuanfu: initialized all un-initialized variables:
      !==================================================
      iback = 0
      itail = 0
      ifun = 0
...
c       On exit m is unchanged.
c
c     sy is a real array of dimension m x m.
c       On entry sy specifies the matrix S'Y.
c       On exit sy is unchanged.
c
c     wt is a real array of dimension m x m.
...
c
c     v is a real array of dimension 2col.
c       On entry v specifies vector v.
c       On exit v is unchanged.
c
c     p is a real array of dimension 2col.
...
c
c     t is a real working array of dimension n. 
c       t will be used to store the break points.
c
c     d is a real array of dimension n used to store
c       the Cauchy direction P(x-tg)-x.
c
c     xcp is a real array of dimension n used to return the
c       GCP on exit.
c
c     m is an integer variable.
c       On entry m is the maximum number of variable metric corrections 
...
c       On exit m is unchanged.
c
c     ws, wy, sy, and wt are real arrays.
...
c
c     wn is a real array of dimension 2m x 2m.
...
c                     [L_a -R_z           theta*S'AA'S ]
c
c     wn1 is a real array of dimension 2m x 2m.
...
c       On exit m is unchanged.
c
c     ws, wy, sy, and wtyy are real arrays;
c     theta is a real variable;
c     col is an integer variable;
c     head is an integer variable.
...
c       On exit n is unchanged.
c
c     t is a real array of dimension n.
...
c
c     wv is a real working array of dimension 2m.
c
c     wn is a real array of dimension 2m x 2m.
...
c         
c       dsave is a real work array of dimension 13.
c
c     Subprograms called
c
c	MINPACK-2 ... dcstep
...
c     *******
      ...
      real a,b,beta,betain,betah,temp,tempa,temp1,
     +       zero,one,two
