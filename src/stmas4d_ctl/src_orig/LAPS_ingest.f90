...
SUBROUTINE get_LAPS_bkgd(num_vars_in,var_names_in,ngrid_in,bkgnd_out, &
                         var_names2d,bkgnd2d,zz)
!doc==================================================================
...
!doc==================================================================
  ...
  CHARACTER*4,  INTENT(IN) :: var_names_in(num_vars_in),var_names2d(1)

  REAL,        INTENT(OUT) :: bkgnd_out(ngrid_in(1),ngrid_in(2), &
                                        ngrid_in(3),ngrid_in(4),num_vars_in), &
                              bkgnd2d(ngrid_in(1),ngrid_in(2),ngrid_in(4)),zz(ngrid_in(3))
...
!
! get surface pressure first
  DO t=1,ngrid_in(4)

! LAPS GET_MODELFG_2D to read lgb 2D data: HJ 6/22/2011
!
    CALL GET_MODELFG_2D(i4time_sequence(t),var_names2d, &
                   ngrid_in(1),ngrid_in(2),bkgnd2d(1,1,t),istatus)

    IF (istatus .NE. 1) THEN

! Use zero as default background field:
       bkgnd2d(1:ngrid_in(1),1:ngrid_in(2),t) = 0.0
! Use background at previous frame if possible:
! HJ add: only if bkbd2d(t-1) not equal to 0. 8/25/2011
       IF (t .GT. 1 .and.  bkgnd2d(1:ngrid_in(1),1:ngrid_in(2),t-1) .ne. 0.0) &
         bkgnd2d(1:ngrid_in(1),1:ngrid_in(2),t) = bkgnd2d(1:ngrid_in(1),1:ngrid_in(2),t-1)
    ELSE
      ! Change unit if necessary
      ! PSF: if in pascal
      IF (var_names2d(1) .EQ. 'PSF' .and. bkgnd2d(1,1,t) .GT. 2000.0) &
        bkgnd2d(1:ngrid_in(1),1:ngrid_in(2),t) = bkgnd2d(1:ngrid_in(1),1:ngrid_in(2),t)/100.
    ENDIF
...
1       FORMAT('STMAS>LAPS_ingest>get_LAPS_bkgd: No LGA background',&
               ' for: ',a4,i3,' Use zero or extrapolation')

! Use zero as default background field:
        bkgnd_out(1:ngrid_in(1),1:ngrid_in(2),1:ngrid_in(3),t,iv) = 0.0
! Use background at previous frame if possible:
        IF (t .GT. 1) &
          bkgnd_out(1:ngrid_in(1),1:ngrid_in(2),1:ngrid_in(3),t,iv) = &
            bkgnd_out(1:ngrid_in(1),1:ngrid_in(2),1:ngrid_in(3),t-1,iv)
      ENDIF
! print sample of bkgnd data. HJ 6/9/2011
     WRITE(*,2) var_names_in(iv),bkgnd_out(2,2,5,t,iv)
2    FORMAT('LAPS_ingest:', A4 , e13.5) 
