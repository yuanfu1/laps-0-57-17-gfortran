...
! HJ add STMAS_mgrid%numgrid(4). 6/10/2011
    ...
    WRITE(*,10) MINVAL(STMAS_mgrid%anal(:,:,:,:,1)),MAXVAL(STMAS_mgrid%anal(:,:,:,:,1)), &
                MINVAL(STMAS_mgrid%anal(:,:,:,:,2)),MAXVAL(STMAS_mgrid%anal(:,:,:,:,2)), &
                MINVAL(STMAS_mgrid%anal(:,:,:,:,3)),MAXVAL(STMAS_mgrid%anal(:,:,:,:,3)), &
                MINVAL(STMAS_mgrid%anal(:,:,:,:,4)),MAXVAL(STMAS_mgrid%anal(:,:,:,:,4)), &
                MINVAL(STMAS_mgrid%anal(:,:,:,:,5)),MAXVAL(STMAS_mgrid%anal(:,:,:,:,5)), mg
  10 FORMAT(/,1X,'STMAS initial guess min max: ', /, &
             ' u min, max: ',E12.5,' ',E12.5, /, &
             ' v min, max: ',E12.5,' ',E12.5, /, &
             ' w min, max: ',E12.5,' ',E12.5, /, &
             ' p min, max: ',E12.5,' ',E12.5, /, &
             ' t min, max: ',E12.5,' ',E12.5, /, &
             ' at mg level: ',I3)
    CALL STMAS_minimizer(num_controls,mg,STMAS_mgrid)
    WRITE(*,11) MINVAL(STMAS_mgrid%anal(:,:,:,:,1)),MAXVAL(STMAS_mgrid%anal(:,:,:,:,1)), &
                MINVAL(STMAS_mgrid%anal(:,:,:,:,2)),MAXVAL(STMAS_mgrid%anal(:,:,:,:,2)), &
                MINVAL(STMAS_mgrid%anal(:,:,:,:,3)),MAXVAL(STMAS_mgrid%anal(:,:,:,:,3)), &
                MINVAL(STMAS_mgrid%anal(:,:,:,:,4)),MAXVAL(STMAS_mgrid%anal(:,:,:,:,4)), &
                MINVAL(STMAS_mgrid%anal(:,:,:,:,5)),MAXVAL(STMAS_mgrid%anal(:,:,:,:,5)), mg
  11 FORMAT(/,1X,'STMAS analysis min max: ', /, &
             ' u min, max: ',E12.5,' ',E12.5, /, &
             ' v min, max: ',E12.5,' ',E12.5, /, &
             ' w min, max: ',E12.5,' ',E12.5, /, &
             ' p min, max: ',E12.5,' ',E12.5, /, &
...
! HJ mod: call to grid4_interpolation. 6/10/2011
    ...
    print*,'final-num',STMAS_final%numgrid,' final pressure: ', &
      MINVAL(STMAS_final%anal(:,:,:,:,4)),MAXVAL(STMAS_final%anal(:,:,:,:,4)), ' bkgd: ', &
      MINVAL(STMAS_final%bkgd(:,:,:,:,4)),MAXVAL(STMAS_final%bkgd(:,:,:,:,4))
