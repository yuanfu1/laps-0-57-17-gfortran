...
! Get background fields: and readin sigma_ht. Otherwise, remove the last argument. HJ 7/29/2011
  CALL get_LAPS_bkgd(STMAS_numvars,STMAS_varnames, &
                     ...
                     STMASFC_varnames,STMAS_final%bkgd2d, &
                     STMAS_final%zz)
!
! HJ add:  6/15/2011
! HJ add: when choosing sigma_ht in the nest7grid.parms, the following
! calls to vertical_z and vinterp_zz are no longer needed. 7/29/2011
!  PRINT*,'------------------------------------------------------------'
!  PRINT*,'|                                                          |'
!  PRINT*,'|    STMAS_LAPS_input: Defining vertical grid,sigma_zz     |'
!  PRINT*,'|                                                          |'
!  PRINT*,'------------------------------------------------------------'
! stmas_final%zz defined here is actually eta (see downscaling code)
! eta=ztop*(zz-topo)/(ztop-topo)
!  CALL vertical_z(STMAS_final%numgrid,STMAS_final%gridspc,STMAS_final%topo, &
...
!  PRINT*,'------------------------------------------------------------'
!  PRINT*,'|                                                          |'
!  PRINT*,'|    STMAS_LAPS_input: Interpreting variables to sigma_zz  |'
!  PRINT*,'|                                                          |'
!  PRINT*,'------------------------------------------------------------'
!  CALL vinterp_zz(STMAS_final%numgrid,STMAS_final%zz,STMAS_final%bkgd,&
!                  STMAS_numvars,STMAS_final%topo,STMAS_final%jt3, &
!                  STMAS_final%bkgd2d)
! using this block instead. HJ 7/29/2011
  PRINT*,'------------------------------------------------------------'
  PRINT*,'|                                                          |'
...
! comment out the call to obs, this part will be written by Yuanfu. HJ 6/6/2011
!hj  PRINT*
!hj  PRINT*,'---------------------------------------------------------'
!hj  PRINT*,'|                                                       |'
!hj  PRINT*,'|           STMAS: Reading observation data             |'
!hj  PRINT*,'|                                                       |'
!hj  PRINT*,'---------------------------------------------------------'
! Get observations:
...
! HJ add t loop 8/18/2011
    ...
    IF (TEMP .NE. 0 .AND. obs%value(iobs,TEMP) .NE. STMAS_invalid .AND. &
      ...
      CALL REDUCE_P(1.8*(obs%value(iobs,TEMP)-temp_0)+32.0, &
                    1.8*(obs%value(iobs,DEWP)-temp_0)+32.0, &
    ...
    !ELSEIF (PRES .NE. 0 .AND. obs%value(iobs,PRES) .NE. STMAS_invalid) THEN
      ...
      ! CALL REDUCE_P(1.8*(temperature-temp_0)+32.0, &
      !               1.8*(dewpoint-temp_0)+32.0, &
