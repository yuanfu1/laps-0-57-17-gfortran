...
!$$$
      ...
      integer(4) :: ieee
      real(4) :: ieee_real(1)
      integer,allocatable :: cpack_int(:)
      real(4),allocatable :: cpack_real(:)
      real :: ref(1),bscale,dscale,unpk(ndpts)
      real,allocatable :: pscale(:)

      ieee = idrstmpl(1)
      ieee_real(1) = transfer(ieee, ieee_real(1))
      call rdieee(ieee_real,ref,1)
      ...
      if (idrstmpl(10).eq.1) then           ! unpacked floats are 32-bit IEEE
         ...
         allocate(cpack_int(Ts))
         allocate(cpack_real(Ts))
         call gbytes(cpack,cpack_int,0,32,0,Ts)
         cpack_real = transfer(cpack_int, cpack_real)
         call rdieee(cpack_real,unpk,Ts)          ! read IEEE unpacked floats
         deallocate(cpack_int)
         deallocate(cpack_real)
         iofst=32*Ts
         ...
         do m=0,MM
            ...
            do n=m,Nm
               if (n.le.Ns .AND. m.le.Ms) then    ! grab unpacked value
                  ...
                  fld(inc)=((real(ifld(incp))*bscale)+ref(1))*
     &                      dscale*pscale(n)           ! real part
                  fld(inc+1)=((real(ifld(incp+1))*bscale)+ref(1))*
