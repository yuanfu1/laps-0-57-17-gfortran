...
C
      ...
      CHARACTER * 1, ALLOCATABLE :: IPFLD_CHAR(:) ! Yuanfu Xie added for XMOVEX type compatibility
      INTEGER         IB(4)
C
...
C
      IF(IBDSFL(2).NE.0) THEN
        ...
        ALLOCATE(IPFLD_CHAR(NPTS*32/BIT_SIZE(1)+1)) ! Yuanfu Xie added for XMOVEX type compatibility
        IPFLD=0
      ENDIF
C
...
C
      DO I=1,SIZE(IPFLD)
        IPFLD_CHAR(I) = CHAR(IPFLD(I))
      END DO
C
      IF(IBDSFL(2).NE.0) THEN
        CALL XMOVEX(PFLD,IPFLD_CHAR,NPTS*4)
        DEALLOCATE(IPFLD, IPFLD_CHAR)
      ENDIF
C
        IF (IBERR .EQ. 1) THEN
          JERR = 3
