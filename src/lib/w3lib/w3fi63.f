...
C
          ALLOCATE(MSGA_INT(500000))
          DO I=1,SIZE(MSGA_INT)
            MSGA_INT(I) = ICHAR(MSGA(I))
          END DO
          CALL GBYTE (MSGA_INT,KKK,KPTR(9)+384,32)
C
...
C
        ...
        END DO
              CALL W3FI01(LW)
              IF (LW.EQ.4) THEN
                  CALL GBYTE (KK_INT,JSGN,0,1)
                  CALL GBYTE (KK_INT,JEXP,1,7)
                  CALL GBYTE (KK_INT,IFR,8,24)
              ELSE
                  CALL GBYTE (KK_INT,JSGN,32,1)
                  CALL GBYTE (KK_INT,JEXP,33,7)
                  CALL GBYTE (KK_INT,IFR,40,24)
              ENDIF
C
              IF (IFR.EQ.0) THEN
                  REALKK = 0.0
...
C
              CALL GBYTE (MSGA_INT,KKK,KPTR(9)+416,32)
C
C       THE NEXT CODE WILL CONVERT THE IBM370 FOATING POINT
C       TO THE FLOATING POINT USED ON YOUR MACHINE.
C
...
C
              ...
              FDIFF1 = REALKK
C
              CALL GBYTE  (MSGA_INT,ISIGN,KPTR(9)+448,1)
              CALL GBYTE  (MSGA_INT,ISCAL2,KPTR(9)+449,15)
              IF(ISIGN.GT.0) THEN
                  ISCAL2 = - ISCAL2
              ENDIF
...
C  -------------------  FIND 'GRIB' KEY
      ...
      END DO
      DO 50 I = 0, 839, 8
          CALL GBYTE (MSGA_INT,MGRIB,I,32)
...
C         PRINT *,'FI631 7777 AT',IPOINT
          ...
          KPTR(1)   = ITOTAL
          KPTR(2)   = 8
          CALL GBYTE (MSGA_INT,KPDS(18),KPTR(8),8)
          KPTR(8)   = KPTR(8) + 8
      ELSE
...
C     PRINT *,'START OF PDS',KPTR(8)
      CALL GBYTE (MSGA_INT,KPTR(3),KPTR(8),24)
      LOOK      = KPTR(8) + 56
C                   EXTRACT GDS/BMS FLAG
      CALL GBYTE (MSGA_INT,KPDS(4),LOOK,8)
      KPTR(8)   = KPTR(8) + KPTR(3) * 8
C     PRINT *,'START OF GDS',KPTR(8)
      IF (IAND(KPDS(4),128).NE.0) THEN
C                   EXTRACT COUNT FROM GDS
          CALL GBYTE (MSGA_INT,KPTR(4),KPTR(8),24)
...
C                   EXTRACT COUNT FROM BMS
          CALL GBYTE (MSGA_INT,KPTR(5),KPTR(8),24)
      ELSE
          KPTR(5)   = 0
      END IF
...
C                   EXTRACT COUNT FROM BDS
      CALL GBYTE (MSGA_INT,KPTR(6),KPTR(8),24)
C  ---------------  TEST FOR '7777'
...
C     PRINT *,'FI631 LOOKING FOR 7777 AT',KPTR(8)
      CALL GBYTE (MSGA_INT,K7777,KPTR(8),32)
...
C                   PARAMETER TABLE VERSION NR
      ...
      DO I=1,SIZE(MSGA_INT)
        MSGA_INT(I) = ICHAR(MSGA(I))
      END DO
          CALL GBYTE (MSGA_INT,KPDS(19),KPTR(8),8)
          KPTR(8)   = KPTR(8) + 8
C  BYTE 5           IDENTIFICATION OF CENTER
      CALL GBYTE (MSGA_INT,KPDS(1),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C  BYTE 6
C                       GET GENERATING PROCESS ID NR
      CALL GBYTE (MSGA_INT,KPDS(2),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C  BYTE 7
C                      GRID DEFINITION
      CALL GBYTE (MSGA_INT,KPDS(3),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C  BYTE 8
C                      GDS/BMS FLAGS
...
C                      INDICATOR OF PARAMETER
      CALL GBYTE (MSGA_INT,KPDS(5),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C  BYTE 10
C                      TYPE OF LEVEL
      CALL GBYTE (MSGA_INT,KPDS(6),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C  BYTE 11,12
C                      HEIGHT/PRESSURE
      CALL GBYTE (MSGA_INT,KPDS(7),KPTR(8),16)
      KPTR(8)   = KPTR(8) + 16
C  BYTE 13
C                      YEAR OF CENTURY
      CALL GBYTE (MSGA_INT,KPDS(8),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C  BYTE 14
C                      MONTH OF YEAR
      CALL GBYTE (MSGA_INT,KPDS(9),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C  BYTE 15
C                      DAY OF MONTH
      CALL GBYTE (MSGA_INT,KPDS(10),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C  BYTE 16
C                      HOUR OF DAY
      CALL GBYTE (MSGA_INT,KPDS(11),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C  BYTE 17
C                      MINUTE
      CALL GBYTE (MSGA_INT,KPDS(12),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C  BYTE 18
C                      INDICATOR TIME UNIT RANGE
      CALL GBYTE (MSGA_INT,KPDS(13),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C  BYTE 19
C                      P1 - PERIOD OF TIME
      CALL GBYTE (MSGA_INT,KPDS(14),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C  BYTE 20
C                      P2 - PERIOD OF TIME
      CALL GBYTE (MSGA_INT,KPDS(15),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C  BYTE 21
C                      TIME RANGE INDICATOR
      CALL GBYTE (MSGA_INT,KPDS(16),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C
...
C                      NUMBER INCLUDED IN AVERAGE
      CALL GBYTE (MSGA_INT,KPDS(17),KPTR(8),16)
      KPTR(8)   = KPTR(8) + 16
C  BYTE 24
C                      NUMBER MISSING FROM AVERAGES/ACCUMULATIONS
      CALL GBYTE (MSGA_INT,KPDS(20),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
C  BYTE 25
C                      IDENTIFICATION OF CENTURY
      CALL GBYTE (MSGA_INT,KPDS(21),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
      IF (KPTR(3).GT.25) THEN
C  BYTE 26              SUB CENTER NUMBER
          CALL GBYTE (MSGA_INT,KPDS(23),KPTR(8),8)
          KPTR(8)   = KPTR(8) + 8
          IF (KPTR(3).GE.28) THEN
C  BYTE 27-28
C                          UNITS DECIMAL SCALE FACTOR
              CALL GBYTE (MSGA_INT,ISIGN,KPTR(8),1)
              KPTR(8)  = KPTR(8) + 1
              CALL GBYTE (MSGA_INT,IDEC,KPTR(8),15)
              KPTR(8)  = KPTR(8) + 15
              IF (ISIGN.GT.0) THEN
                  KPDS(22)  = - IDEC
              ELSE
              ...
              IF (ISIZ.LE.12) THEN
C  BYTE  29
                  CALL GBYTE (MSGA_INT,KPDS(24),KPTR(8)+8,8)
C  BYTE  30
                  CALL GBYTE (MSGA_INT,KPDS(25),KPTR(8)+16,8)
...
C  BYTES 41 - N                 LOCAL USE DATA
                  ...
                  CALL GBYTES (MSGA_INT,KPDS(36),KPTR(8),MWDBIT,0,ITER)
                  KPTR(8)  = KPTR(8) + ISIZ * 8
              END IF
          END IF
      END IF
...
C                   NV - NR OF VERT COORD PARAMETERS
      ...
      DO I=1,SIZE(MSGA_INT)
        MSGA_INT(I) = ICHAR(MSGA(I))
      END DO
      CALL GBYTE (MSGA_INT,KGDS(19),KPTR(8),8)
      KPTR(8)  = KPTR(8) + 8
C  BYTE 5
C                   PV - LOCATION - SEE FM92 MANUAL
      CALL GBYTE (MSGA_INT,KGDS(20),KPTR(8),8)
      KPTR(8)  = KPTR(8) + 8
C  BYTE 6
C                      DATA REPRESENTATION TYPE
      CALL GBYTE (MSGA_INT,KGDS(1),KPTR(8),8)
      KPTR(8)   = KPTR(8) + 8
...
C                                       BETWEEN POLE AND EQUATOR
      CALL GBYTE (MSGA_INT,KGDS(10),KPTR(8),16)
      KPTR(8)  = KPTR(8) + 16
C  ------------------- BYTE 28      SCANNING MODE FLAGS
      CALL GBYTE (MSGA_INT,KGDS(11),KPTR(8),8)
      KPTR(8)  = KPTR(8) + 8
C  ------------------- BYTE 29-32   RESERVED
C                             SKIP TO START OF BYTE 33
      CALL GBYTE (MSGA_INT,KGDS(12),KPTR(8),32)
      KPTR(8)  = KPTR(8) + 32
C  -------------------
      GO TO 900
C  ******************************************************************
...
C  ------------------- BYTE 11-12   M PENTAGONAL RESOLUTION PARAMETER
      CALL GBYTE (MSGA_INT,KGDS(4),KPTR(8),16)
      KPTR(8)  = KPTR(8) + 16
C  ------------------- BYTE 13 REPRESENTATION TYPE
      CALL GBYTE (MSGA_INT,KGDS(5),KPTR(8),8)
      KPTR(8)  = KPTR(8) + 8
...
C
      IF (KGDS(19).EQ.0.OR.KGDS(19).EQ.255) THEN
        IF (KGDS(20).NE.255) THEN
          ...
          CALL GBYTES (MSGA_INT,KGDS(22),KPTR(8),16,0,KGDS(3))
          DO 910 J = 1, KGDS(3)
              ISUM  = ISUM + KGDS(21+J)
  910     CONTINUE
          KGDS(21)  = ISUM
...
C
      ...
      DO I=1,SIZE(MSGA_INT)
         MSGA_INT(I)=ICHAR(MSGA(I))
      END DO
      CALL GBYTE (MSGA_INT,KPTR(11),KPTR(8),8)
      KPTR(8)  = KPTR(8) + 8
C
C  BYTE 5,6         TABLE REFERENCE IF 0, BIT MAP FOLLOWS
C
      CALL GBYTE (MSGA_INT,KPTR(12),KPTR(8),16)
      KPTR(8)  = KPTR(8) + 16
...
C$$$
      CHARACTER*1   MSGA(*)
      INTEGER MSGA_INT(NPTS)
      LOGICAL*1     KBMS(NPTS)
      INTEGER       ICHK(NPTS)
C - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      DO I=1,NPTS
        MSGA_INT(I)=ICHAR(MSGA(NSKP+(I-1)/8+1))
      END DO
      CALL GBYTES(MSGA_INT,ICHK,NSKP,1,0,NPTS)
      KBMS=ICHK.NE.0
C - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      END
...
C
      CHARACTER*1   MSGA(*)
      INTEGER, ALLOCATABLE :: MSGA_INT(:) ! Yuanfu Xie added this to solve the conflicting data types called in GBYTE
      CHARACTER*1   KK(8)
      INTEGER       KK_INT(8) ! Yuanfu Xie added this to solve the conflicting data types called in GBYTE
      CHARACTER*1   CKREF(8)
      INTEGER       CKREF_INT(8) ! Yuanfu Xie added this to solve the conflicting data types called in GBYTE
C
      LOGICAL*1     KBMS(*)
C
      INTEGER       KPDS(*)
...
C            BYTE 4
      ...
      DO I=1,500000
        MSGA_INT(I) = ICHAR(MSGA(I))
      END DO
      CALL GBYTE(MSGA_INT,KPTR(14),KPTR(8),4)
      KPTR(8)  = KPTR(8) + 4
C  --------- NR OF UNUSED BITS IN SECTION 4
      CALL GBYTE(MSGA_INT,KPTR(15),KPTR(8),4)
      KPTR(8)  = KPTR(8) + 4
...
C            BYTES 5,6
C                                  CHECK SIGN
      CALL GBYTE (MSGA_INT,KSIGN,KPTR(8),1)
      KPTR(8)  = KPTR(8) + 1
C                                  GET ABSOLUTE SCALE VALUE
      CALL GBYTE (MSGA_INT,KSCALE,KPTR(8),15)
      KPTR(8)  = KPTR(8) + 15
      IF (KSIGN.GT.0) THEN
          KSCALE  = - KSCALE
      END IF
...
C            BYTES 7,10
      CALL GBYTE (MSGA_INT,KREF,KPTR(8),32)
      KPTR(8)  = KPTR(8) + 32
C
...
C
      DO I=1,8
        CKREF_INT(I) = ICHAR(CKREF(I))
      END DO
      CALL W3FI01(LW)
      IF (LW.EQ.4) THEN
        CALL GBYTE (CKREF_INT,JSGN,0,1)
        CALL GBYTE (CKREF_INT,JEXP,1,7)
        CALL GBYTE (CKREF_INT,IFR,8,24)
      ELSE
        CALL GBYTE (CKREF_INT,JSGN,32,1)
        CALL GBYTE (CKREF_INT,JEXP,33,7)
        CALL GBYTE (CKREF_INT,IFR,40,24)
      ENDIF
...
C            BYTE 11
      CALL GBYTE (MSGA_INT,KBITS,KPTR(8),8)
      KPTR(8)  = KPTR(8) + 8
      KBDS(4)  = KBITS
C     KBDS(13) = KBITS
      KPTR(20) = KBITS
...
C         PRINT *,'NO EXTENDED FLAGS'
      ELSE
C            BYTES 12,13
          CALL GBYTE (MSGA_INT,KOCTET,KPTR(8),16)
          KPTR(8)  = KPTR(8) + 16
C  --------------------------- EXTENDED FLAGS
C            BYTE 14
          CALL GBYTE (MSGA_INT,KXFLAG,KPTR(8),8)
...
C            BYTES 15,16
          CALL GBYTE (MSGA_INT,NR,KPTR(8),16)
          KPTR(8)  = KPTR(8) + 16
C  ---------------------- NC - SECOND DIMENSION (COLS) OF EACH MATRIX
C            BYTES 17,18
          CALL GBYTE (MSGA_INT,NC,KPTR(8),16)
          KPTR(8)  = KPTR(8) + 16
C  ---------------------- NRV - FIRST DIM COORD VALS
C            BYTE 19
          CALL GBYTE (MSGA_INT,NRV,KPTR(8),8)
          KPTR(8)  = KPTR(8) + 8
C  ---------------------- NC1 - NR COEFF'S OR VALUES
C            BYTE 20
          CALL GBYTE (MSGA_INT,NC1,KPTR(8),8)
          KPTR(8)  = KPTR(8) + 8
C  ---------------------- NCV - SECOND DIM COORD OR VALUE
C            BYTE 21
          CALL GBYTE (MSGA_INT,NCV,KPTR(8),8)
          KPTR(8)  = KPTR(8) + 8
C  ---------------------- NC2 - NR COEFF'S OR VALS
C            BYTE 22
          CALL GBYTE (MSGA_INT,NC2,KPTR(8),8)
          KPTR(8)  = KPTR(8) + 8
C  ---------------------- KPHYS1 - FIRST DIM PHYSICAL SIGNIF
C            BYTE 23
          CALL GBYTE (MSGA_INT,KPHYS1,KPTR(8),8)
          KPTR(8)  = KPTR(8) + 8
C  ---------------------- KPHYS2 - SECOND DIM PHYSICAL SIGNIF
C            BYTE 24
          CALL GBYTE (MSGA_INT,KPHYS2,KPTR(8),8)
          KPTR(8)  = KPTR(8) + 8
C            BYTES 25-N
      END IF
      IF (KBITS.EQ.0) THEN
...
C
     ...
     *            OR.KPDS(3).EQ.63.OR.KPDS(3).EQ.64) THEN
          ...
          ELSE
              KADD    = 37
          END IF
          CALL GBYTES (MSGA_INT,KSAVE,KPTR(8),KBITS,0,KNR)
...
C     PRINT *,'CHECK POINT SPECTRAL COEFF'
      KPTR(8)  = IBYT12
      CALL GBYTE (MSGA_INT,KKK,KPTR(8),32)
      KPTR(8)  = KPTR(8) + 32
C
C     THE NEXT CODE WILL CONVERT THE IBM370 FOATING POINT
C     TO THE FLOATING POINT USED ON YOUR MACHINE.
...
C                    BINARY SCALE VALUE  BDS BYTES 5-6
      ALLOCATE(MSGA_INT(500000)) ! Yuanfu Xie added this to solve the conflicting data types called in GBYTE)
      DO I=1,SIZE(MSGA_INT)
        MSGA_INT(I) = ICHAR(MSGA(I))
      END DO
      CALL GBYTE (MSGA_INT,ISIGN,JPTR+32,1)
      CALL GBYTE (MSGA_INT,KBDS(11),JPTR+33,15)
      IF (ISIGN.GT.0) THEN
          KBDS(11)  = - KBDS(11)
      END IF
C     PRINT *,'BINARY SCALE VALUE =',KBDS(11)
C                  EXTRACT REFERENCE VALUE
      CALL GBYTE(MSGA_INT,JREF,JPTR+48,32)
C     PRINT *,'DECODED REFERENCE VALUE =',REFN,REFNCE
C                F O BIT WIDTH
      CALL GBYTE(MSGA_INT,KBDS(13),JPTR+80,8)
      JPTR  = JPTR + 88
C              AT START OF BDS BYTE 12
C                EXTRACT N1
      CALL GBYTE (MSGA_INT,KBDS(1),JPTR,16)
C     PRINT *,'N1  = ',KBDS(1)
      JPTR  = JPTR + 16
C                 EXTENDED FLAGS
      CALL GBYTE (MSGA_INT,KFLAG,JPTR,8)
C                 ISOLATE BIT MAP FLAG
      ...
      JPTR  = JPTR + 8
C                EXTRACT N2
      CALL GBYTE (MSGA_INT,KBDS(2),JPTR,16)
C     PRINT *,'N2  = ',KBDS(2)
      JPTR  = JPTR + 16
C                EXTRACT P1
      CALL GBYTE (MSGA_INT,KBDS(3),JPTR,16)
C     PRINT *,'P1  = ',KBDS(3)
      JPTR  = JPTR + 16
C                EXTRACT P2
      CALL GBYTE (MSGA_INT,KBDS(4),JPTR,16)
C     PRINT *,'P2  = ',KBDS(4)
      JPTR  = JPTR + 16
...
C                               GET NUMBER IN CURRENT ROW
                      CALL GBYTE (MSGA_INT,NUMBER,LP,16)
...
C 900         FORMAT (1X,I4,3X,14HMAIN BIT IS ON,3X,L4)
              IF (KBDS(14).NE.0) THEN
                  CALL GBYTE (MSGA_INT,KBIT,KBDS(6),1)
              ELSE
...
C                                  GET NEXT FIRST ORDER PACKED VALUE
                  CALL GBYTE (MSGA_INT,IFOVAL,KBDS(7),KBDS(13))
                  KBDS(7)  = KBDS(7) + KBDS(13)
C                 PRINT *,'FOVAL =',IFOVAL
C                                   GET SECOND ORDER BIT WIDTH
                  CALL GBYTE (MSGA_INT,KBDS(15),KBDS(5),8)
                  KBDS(5)  = KBDS(5) + 8
...
C                 DATA(I) = (REFNCE + (FLOAT(IFOVAL) * SCALE2)) / SCAL10
              ELSE
                  CALL GBYTE (MSGA_INT,ISOVAL,KBDS(8),KBDS(15))
