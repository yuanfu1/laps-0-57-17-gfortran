...
C
      ...
      INTEGER, ALLOCATABLE :: BDS_INT(:)        ! Yuanfu Xie added this line to fix
                                                ! compilation error on calling sbyte
      CHARACTER * 1   PFLD(*)
      CHARACTER * 1   CIEXP(8)
...
C                                          ON 32 OR 64 BIT COMPUTER
              ...
              ! Yuanfu Xie modified here to fix compilation error on calling sbyte
              ALLOCATE(BDS_INT(IPDSIZ))
              BDS_INT = 0
              DO I=1,IPDSIZ
                 BDS_INT(I) = ICHAR(PDS(I))
              ENDDO
              CALL SBYTE (BDS_INT,IEXP,320,8)
              CALL SBYTE (BDS_INT,IMANT,328,24)
C
              IF (LW.EQ.4) THEN
              ...
              END IF
              CALL SBYTE (BDS_INT,IEXP,352,8)
              CALL SBYTE (BDS_INT,IMANT,360,24)
C
C             TURN ISCAL2 INTO SIGNED POSITIVE INTEGER
C             AND STORE IN TWO BYTES
C
              IF(ISCAL2.GE.0)  THEN
                CALL SBYTE (BDS_INT,ISCAL2,384,16)
              ELSE
                CALL SBYTE (BDS_INT,1,384,1)
                ISCAL2 = - ISCAL2
                CALL SBYTE( BDS_INT,ISCAL2,385,15)
              ENDIF
              DO I=1,IPDSIZ
                 PDS(I) = CHAR(BDS_INT(I))
              ENDDO
C
              MAX  = IWORK(1)
              MIN  = IWORK(1)
...
C
C                               BYTES 1-3
          IF (ALLOCATED(BDS_INT)) DEALLOCATE(BDS_INT)
          ALLOCATE(BDS_INT(LENBDS))
          DO I=1,LENBDS
            BDS_INT(I) = ICHAR(BDS11(I))
          END DO
          CALL SBYTE (BDS_INT,LENBDS,0,24)
C
C                               BYTE  4
C                                       FLAGS
          CALL SBYTE (BDS_INT,IBDSFL(1),24,1)
          CALL SBYTE (BDS_INT,IBDSFL(2),25,1)
          CALL SBYTE (BDS_INT,IBDSFL(3),26,1)
          CALL SBYTE (BDS_INT,IBDSFL(4),27,1)
C                                        NR OF FILL BITS
          CALL SBYTE (BDS_INT,NFILL,28,4)
C
C$      FILL OCTETS 5-6 WITH THE SCALE FACTOR.
C
C                               BYTE  5-6
          IF (ISCALE.LT.0) THEN
              CALL SBYTE (BDS_INT,1,32,1)
              ISCALE  = - ISCALE
              CALL SBYTE (BDS_INT,ISCALE,33,15)
          ELSE
              CALL SBYTE (BDS_INT,ISCALE,32,16)
          END IF
C
...
C                                          ON 32 OR 64 BIT COMPUTER
          ...
          END IF
          CALL SBYTE (BDS_INT,IEXP,48,8)
          CALL SBYTE (BDS_INT,IMANT,56,24)
C
C
C$                        FILL OCTET 11 WITH THE NUMBER OF BITS.
C
C                               BYTE  11
          CALL SBYTE (BDS_INT,NBITS,80,8)
      END IF
      DO I=1,LENBDS
        BDS11(I) = CHAR(BDS_INT(I))
      END DO
C
      RETURN
      END
...
C                                   WHEN LENGTH OF BDS IS KNOWN
      ...
      ! Yuanfu Xie fixed the calls of sbyte to avoid compilation error on stricter type check:
      ALLOCATE (BDS_INT(LENBDS))
      DO I = 1, LENBDS
         BDS_INT(I) = ICHAR(BDS11(I))
      ENDDO
...
C                              ENTER NUMBER OF FILL BITS
      CALL SBYTE (BDS_INT,NLEFT,28,4)
C                               BYTE  5-6
      IF (ISCAL2.LT.0) THEN
          CALL SBYTE (BDS_INT,1,32,1)
          ISCAL2 = - ISCAL2
      ELSE
          CALL SBYTE (BDS_INT,0,32,1)
      END IF
      CALL SBYTE (BDS_INT,ISCAL2,33,15)
C
C$  FILL OCTET 7-10 WITH THE REFERENCE VALUE
C   CONVERT THE FLOATING POINT OF YOUR MACHINE TO IBM370 32 BIT
C   FLOATING POINT NUMBER
...
C                               BYTE  11
C
      CALL SBYTE (BDS_INT,KBDS(11),80,8)
