...
!mp-BLS
    ...
    status = NF_GET_VAR_REAL(cdfid , vid , dy)    ! Yuanfu Xie modified by using dy, a rank-1 array, to call stdlon
    stdlon = dy(1)
    ...
    status = NF_GET_VAR_REAL( cdfid , vid , dy) ! Yuanfu Xie modified by using dy, a rank-1 array, to call truelat1 
    truelat1 = dy(1)
    status = NF_INQ_VARID(cdfid , NAME_TRUELAT2, vid)
    status = NF_GET_VAR_REAL( cdfid , vid , dy) ! Yuanfu Xie modified by using dy, a rank-1 array, to call truelat2
    truelat2 = dy(1)
