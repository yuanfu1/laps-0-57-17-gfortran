...
CONTAINS
  ...
  SUBROUTINE wrf2lga(wrffile,i4time,cmodel,istatus )
    ...
    ! Allocate static fields
     ...
     jcentw = nyw/2 
     ! Get WRF on sigma
    IF (.NOT. ALLOCATED(pr_wrfs))  ALLOCATE (pr_wrfs(nxw,nyw,nzw)) 
    IF (.NOT. ALLOCATED(ht_wrfs))  ALLOCATE (ht_wrfs(nxw,nyw,nzw))
    IF (.NOT. ALLOCATED(t3_wrfs))  ALLOCATE (t3_wrfs(nxw,nyw,nzw))
    IF (.NOT. ALLOCATED(sh_wrfs))  ALLOCATE (sh_wrfs(nxw,nyw,nzw)) 
    IF (.NOT. ALLOCATED(u3_wrfs))  ALLOCATE (u3_wrfs(nxw,nyw,nzw))
    IF (.NOT. ALLOCATED(v3_wrfs))  ALLOCATE (v3_wrfs(nxw,nyw,nzw))
    IF (.NOT. ALLOCATED(om_wrfs))  ALLOCATE (om_wrfs(nxw,nyw,nzw)) 
    IF (.NOT. ALLOCATED(mr_wrfs))  ALLOCATE (mr_wrfs(nxw,nyw,nzw))
    IF (.NOT. ALLOCATED(rho_wrfs))  ALLOCATE (rho_wrfs(nxw,nyw,nzw))
    IF (.NOT. ALLOCATED(usf_wrf))  ALLOCATE (usf_wrf(nxw,nyw))
    IF (.NOT. ALLOCATED(vsf_wrf))  ALLOCATE (vsf_wrf(nxw,nyw))
    IF (.NOT. ALLOCATED(tsf_wrf))  ALLOCATE (tsf_wrf(nxw,nyw))
    IF (.NOT. ALLOCATED(tsk_wrf))  ALLOCATE (tsk_wrf(nxw,nyw)) ! surface skin temp. (added by Wei-Ting 130312)
    IF (.NOT. ALLOCATED(rsf_wrf))  ALLOCATE (rsf_wrf(nxw,nyw))
    IF (.NOT. ALLOCATED(dsf_wrf))  ALLOCATE (dsf_wrf(nxw,nyw))
    IF (.NOT. ALLOCATED(slp_wrf))  ALLOCATE (slp_wrf(nxw,nyw))
    IF (.NOT. ALLOCATED(psf_wrf))  ALLOCATE (psf_wrf(nxw,nyw))
    IF (.NOT. ALLOCATED(p_wrf))    ALLOCATE (p_wrf(nxw,nyw))
    IF (.NOT. ALLOCATED(pcp_wrf))  ALLOCATE (pcp_wrf(nxw,nyw)) ! RAINNC+RAINC (added by Wei-Ting 130312)
    IF (.NOT. ALLOCATED(topo_wrf))  ALLOCATE (topo_wrf(nxw,nyw))
    IF (.NOT. ALLOCATED(tvb_wrf))  ALLOCATE (tvb_wrf(nxw,nyw))
     ...
     PRINT *, "Allocating arrays for WRF on Pressure"
       ! Allocate wrfp
    IF (.NOT. ALLOCATED(ht_wrfp))  ALLOCATE (ht_wrfp(nxw,nyw,nzl))
    IF (.NOT. ALLOCATED(t3_wrfp))  ALLOCATE (t3_wrfp(nxw,nyw,nzl))
    IF (.NOT. ALLOCATED(sh_wrfp))  ALLOCATE (sh_wrfp(nxw,nyw,nzl))
    IF (.NOT. ALLOCATED(u3_wrfp))  ALLOCATE (u3_wrfp(nxw,nyw,nzl))
    IF (.NOT. ALLOCATED(v3_wrfp))  ALLOCATE (v3_wrfp(nxw,nyw,nzl))
    IF (.NOT. ALLOCATED(om_wrfp))  ALLOCATE (om_wrfp(nxw,nyw,nzl))
     ...
     IF ( ALLOCATED(pr_wrfs)) DEALLOCATE(pr_wrfs)
     IF ( ALLOCATED(ht_wrfs)) DEALLOCATE(ht_wrfs)
     IF ( ALLOCATED(t3_wrfs)) DEALLOCATE(t3_wrfs)
     IF ( ALLOCATED(u3_wrfs)) DEALLOCATE(u3_wrfs)
     IF ( ALLOCATED(v3_wrfs)) DEALLOCATE(v3_wrfs)
     IF ( ALLOCATED(om_wrfs)) DEALLOCATE(om_wrfs)
     IF ( ALLOCATED(mr_wrfs)) DEALLOCATE(mr_wrfs)
     IF ( ALLOCATED(rho_wrfs)) DEALLOCATE(rho_wrfs)
     ...
     IF (.NOT. ALLOCATED(tsk)) ALLOCATE (tsk(nxl,nyl)) ! surface skin temp. (added by Wei-Ting 130312)
     ...
     IF (.NOT. ALLOCATED(pcp)) ALLOCATE (pcp(nxl,nyl)) ! precitation (added by Wei-Ting 130312)
     usf = rmissingflag
     vsf = rmissingflag
     tsf = rmissingflag
...
!     pcp = 0 ! since pcp hasn't be used for now, assume that the value is 0
    ...
    IF ( ALLOCATED(usf_wrf)) DEALLOCATE (usf_wrf)
    IF ( ALLOCATED(vsf_wrf)) DEALLOCATE (vsf_wrf)
    IF ( ALLOCATED(tsf_wrf)) DEALLOCATE (tsf_wrf)
    IF ( ALLOCATED(tsk_wrf)) DEALLOCATE (tsk_wrf)
    IF ( ALLOCATED(rsf_wrf)) DEALLOCATE (rsf_wrf)
    IF ( ALLOCATED(dsf_wrf)) DEALLOCATE (dsf_wrf)
    IF ( ALLOCATED(slp_wrf)) DEALLOCATE (slp_wrf)
    IF ( ALLOCATED(psf_wrf)) DEALLOCATE (psf_wrf)
    IF ( ALLOCATED(p_wrf)) DEALLOCATE (p_wrf)
    IF ( ALLOCATED(pcp_wrf)) DEALLOCATE (pcp_wrf)
    IF ( ALLOCATED(tvb_wrf)) DEALLOCATE (tvb_wrf) ! added tsk_wrf & pcp_wrf by Wei-Ting (130312)
    ...
    IF ( ALLOCATED(topo_wrf)) DEALLOCATE (topo_wrf)
     ! Write the data out 
    print *, "Writing LGA"
!      CALL write_lga(nxl,nyl,nzl,bg_valid,bg_valid,cmodel,rmissingflag, &
     CALL write_lga(nxl,nyl,nzl,i4reftime,bg_valid,cmodel,rmissingflag, & ! xhb
...
!     CALL write_lgb(nxl,nyl,bg_valid,bg_valid,cmodel,rmissingflag, &
     CALL write_lgb(nxl,nyl,i4reftime,bg_valid,cmodel,rmissingflag, &   ! xhb     
     ...
     IF ( ALLOCATED(pcp)) DEALLOCATE(pcp) ! added tsk & pcp by Wei-Ting (130312)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  IF (ALLOCATED(topo_wrfl))  DEALLOCATE(topo_wrfl)
