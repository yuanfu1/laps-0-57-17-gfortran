...
SUBROUTINE wrfpost_get_basic()
  ...
  IF (.NOT. ALLOCATED(d1)) ALLOCATE (d1 (nx,ny,nz) )
  IF (.NOT. ALLOCATED(d2)) ALLOCATE (d2 (nx,ny,nz) )
  IF (.NOT. ALLOCATED(d1f)) ALLOCATE (d1f (nx,ny,nz+1) )
  IF (.NOT. ALLOCATED(d2f)) ALLOCATE (d2f (nx,ny,nz+1) )
  ...
  IF( ALLOCATED(d1f))  DEALLOCATE(d1f)
  ...
  PRINT *, '  -- Precip Rate'
  IF( .NOT. ALLOCATED(dum2d) ) ALLOCATE(dum2d(nx,ny))
  ...
  ELSE
    ...
    IF( ALLOCATED(pcprate) ) DEALLOCATE(pcprate)
  ENDIF
  ...
  IF( .NOT. ALLOCATED(rh_2m) ) ALLOCATE(rh_2m(nx,ny))
  IF( .NOT. ALLOCATED(td_2m) ) ALLOCATE(td_2m(nx,ny))
  IF( .NOT. ALLOCATED(rh3) ) ALLOCATE(rh3(nx,ny,nz))
  ...
  IF( .NOT. ALLOCATED(wdir_10) ) ALLOCATE(wdir_10m(nx,ny))
  IF( .NOT. ALLOCATED(wspd_10) ) ALLOCATE(wspd_10m(nx,ny))
  DO j = 1, ny
    DO i = 1, nx
  ...
  IF( .NOT. ALLOCATED(mslp) ) ALLOCATE(mslp(nx,ny))
  IF( .NOT. ALLOCATED(tv_sl)) ALLOCATE(tv_sl(nx,ny))
  IF( .NOT. ALLOCATED(z_1000) ) ALLOCATE(z_1000(nx,ny))
