...
c    &         pressure_pa, prsQcFlag,                            ! O 
     ...
     &         istatus)                                           ! O
      ...
      integer       istatus
      integer       nsmQcFlag  ! dimension size for smQcFlag
...
!       return
      ...
      if(istat_stationPressure .eq. 1)then
        nf_status = NF_INQ_VARID(nf_fid,'prsQcFlag',prsQc_id)
        if(nf_status.ne.NF_NOERR) then
          print *, NF_STRERROR(nf_status)
          print *,'finding var prsQcFlag'
          print *, 'Aborting read'
          nf_status = NF_CLOSE(nf_fid)
          istatus = 0
          return
...
c             read prsQcFlag
              nf_status=NF_GET_VAR1_INT(nf_fid,prsQc_id,index_1,prsQF)    
              if(nf_status.ne.NF_NOERR) then
                print *, NF_STRERROR(nf_status)
...
c           read var tempQcFlag(recNum,lvl) -> tempQcFlag
            if(istat_tempQcFlag .eq. 1)then
                nf_status = NF_GET_VAR1_INT(nf_fid,tempQc_id,index_2,
     1                                       tempQF)
                if(nf_status.ne.NF_NOERR) then
                  print *, NF_STRERROR(nf_status)
...
c           read var rhQcFlag(recNum,level) -> rhQcFlag
            nf_status=NF_GET_VAR1_INT(nf_fid,rhQc_id,index_2,rhQF)
            if(nf_status.ne.NF_NOERR) then
              print *, NF_STRERROR(nf_status)
...
c           read var wsQcFlag(recNum,level) -> wsQcFlag
            nf_status=NF_GET_VAR1_INT(nf_fid,wsQc_id,index_2,wsQF)
            if(nf_status.ne.NF_NOERR) then
              print *, NF_STRERROR(nf_status)
...
c           read var wdQcFlag(recNum,level) -> wdQcFlag(lvl,obno)
            nf_status=NF_GET_VAR1_INT(nf_fid,wdQc_id,index_2,wdQF)
            if(nf_status.ne.NF_NOERR) then
              print *, NF_STRERROR(nf_status)
