...
#-------------------------------------------------------
#       $command = "/usr/bin/perl $INSTALL_ROOT/etc/purge_wideband.pl $purgewl $purgewh $wideband_2_netcdf";
#       if(-e $wideband_2_netcdf){
...
#                    if($i4time70_driver == $i4time70 && $laps_driver_exe =~ /^remap/){last;}
                     ...
                     if(defined $multi_machine){
                        $com_ld="/usr/bin/perl $INSTALL_ROOT/etc/systime.pl $yr2 $mo2 $dy2 $hr2 $mn2 > /tmp/time/systime.dat";
                     }else{
                        $com_ld="/usr/bin/perl $INSTALL_ROOT/etc/systime.pl $yr2 $mo2 $dy2 $hr2 $mn2 > $DATA_ROOT/time/systime.dat";
                     }
                     run_sys::run_sys($com_ld);

                     if(defined $driverargs){
                         $com_ld="/usr/bin/perl $INSTALL_ROOT/etc/laps_driver.pl -L $dy2-$mon2-$year2-$hr2$mn2 $driverargs $laps_driver_exe $INSTALL_ROOT $DATA_ROOT";
                     }else{
                         $com_ld="/usr/bin/perl $INSTALL_ROOT/etc/laps_driver.pl -L $dy2-$mon2-$year2-$hr2$mn2             $laps_driver_exe $INSTALL_ROOT $DATA_ROOT";
                     }
     ...
     if(defined $multi_machine){
        $command = "/usr/bin/perl $INSTALL_ROOT/etc/systime.pl $yr $mo $dy $hr $mn > /tmp/time/systime.dat";
     }else{
        $command = "/usr/bin/perl $INSTALL_ROOT/etc/systime.pl $yr $mo $dy $hr $mn > $DATA_ROOT/time/systime.dat";
     }
     print "\n";
...
#---------------
     ...
     if(defined $runsched)
     {
        ...
        $command="/usr/bin/perl $INSTALL_ROOT/etc/sched.pl -A $dy-$mon-$year-$hr$mn $runsched $INSTALL_ROOT $DATA_ROOT";
        run_sys::run_sys($command);
     }
