#!/usr/bin/perl
# Generated automatically from sched.pl.in by configure.
...
#                   print TFILE "module load perlmod\n";
                ...
                if($node_type eq "rt"){
                    print TFILE "export PBS_SERVER_LIST=/usr/local/fsl/lib/server.rt\n";
                    if($opt_q){
...
#           added -t $yyjjjhhmm to allow data from systime.dat at time sched.pl runs to be passed into wfo_post.pl (allows post processing to run overtime)
            if($opt_w){ # Bigfile option within qsub environment
                print TFILE "/usr/bin/perl $LAPSROOT/etc/wfo_post.pl -w -t $yyjjjhhmm $LAPSROOT $LAPS_DATA_ROOT 1> $LAPS_LOG_PATH/wfo_post.log.$hhmm 2>&1\n";
                print TFILE "  \n";
            }
        }
...
#           print TFILE "  exit \${error}\n";
        ...
        if(-e "$LAPSROOT/etc/$wgi_script_name.pl"){
           if( $wgi_script_name eq "sfc"){
               $exe_name = $1 if($exe =~ /^(\S+)\./);
               print TFILE "/usr/bin/perl $LAPSROOT/etc/$wgi_script_name.pl -r $exe_name $LAPS_DATA_ROOT $yyjjjhhmm 1> $LAPS_LOG_PATH/$wgi_script_name.wgi.$yyjjjhhmm 2>&1\n";
           }else{
               print TFILE "/usr/bin/perl $LAPSROOT/etc/$wgi_script_name.pl $LAPS_DATA_ROOT $yyjjjhhmm 1> $LAPS_LOG_PATH/$wgi_script_name.wgi.$yyjjjhhmm 2>&1\n";
           }
        }
...
#       system("/usr/nfs/bin/perl $LAPSROOT/etc/wfo_post.pl $LAPSROOT $LAPS_DATA_ROOT 1> $LAPS_LOG_PATH/wfo_post.log.$hhmm 2>&1");
  ...
  if (index($LAPS_DATA_ROOT,"usfsfire/f1") != -1) {
    ...
    $sys = "/usr/bin/perl $LAPSROOT/etc/putFile_fire1.pl $LAPSROOT $filename";
    system("$sys");
