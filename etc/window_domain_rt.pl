...
#
# opt_w is which type (laps, wrfsi, or wrfsi.rotlat).
#
...
if(!defined $opt_w){die "\nYou must decide and use the -w command line input to
                           specify the type of localization desired. Three options:
                           1. -w laps
                           2. -w wrfsi
                           3. -w wrfsi.rotlat\n\n";}
...
if(! defined($LAPS_DATA_ROOT) ){$LAPS_DATA_ROOT = "$LAPSINSTALLROOT/data";
   if($LAPS_SRC_ROOT eq $LAPSINSTALLROOT){
      $all_roots_equal = "T";
      print "WARNING: All lapsroots are equal - exiting\n";
      exit;
   }
}
...
print "Input to laps_data_root module (LAPS_DATA_ROOT) is $LAPS_DATA_ROOT\n";
$DATAROOT = &laps_tools::laps_data_root($LAPS_DATA_ROOT);
print "Output from laps_data_root module (DATAROOT) is $DATAROOT\n";
...
#  Check for template that hasn't been separated
   if($LAPS_TEMPLATE =~ $LAPS_SRC_ROOT || $LAPS_SRC_ROOT =~ $LAPS_TEMPLATE){
      print "Error: Template should be separated from SRCROOT\n";
      print "Template is $LAPS_TEMPLATE\n";
      print "SRCROOT is $LAPS_SRC_ROOT\n";
      print "Terminating.\n";exit;
      exit;
   }

   if($LAPS_TEMPLATE =~ $LAPSINSTALLROOT || $LAPSINSTALLROOT =~ $LAPS_TEMPLATE){
      print "Error: Template should be separated from INSTALLROOT\n";
      print "Template is $LAPS_TEMPLATE\n";
      print "INSTALLROOT is $LAPSINSTALLROOT\n";
      print "Terminating.\n";exit;
      exit;
   }

   if($LAPS_TEMPLATE =~ $DATAROOT || $DATAROOT =~ $LAPS_TEMPLATE){
      print "Error: Template should be separated from DATAROOT\n";
      print "Template is $LAPS_TEMPLATE\n";
      print "DATAROOT is $DATAROOT\n";
      print "Terminating.\n";exit;
      exit;
   }
...
#   print "-c command line not specified. Default set to false \n";
    ...
    &laps_tools::mkdatadirs($LAPS_DATA_ROOT,$LAPS_SRC_ROOT,$domain_type);
}
...
#test if first time "data" and link need to be established
   if( ! -d "$FXA_DATA/laps_data"){
       mkdir "$FXA_DATA/laps_data", 0777 or die "Can't make directory $FXA_DATA/laps_data: if non-AWIPS run then please unsetenv $FXA_DATA";
...
# First check to see if there are leftover static_save and cdl_save.
        ...
        if(-e "$LAPS_DATA_ROOT/static\_save"){
           print "Warning: found static_save after attempted removal\n";
        }
...
# Move existing static and cdl to "_save" and prepare new ones!
        ...
        if(-e "$LAPS_DATA_ROOT/static"){
           print "Warning: static directory is still there after renaming\n";
        } 
        ...
        print "All roots are equal. Try placing LAPS_DATA_ROOT in a directory tree separate from LAPSINSTALLROOT.\n";
...
# -----------------------------------
if($domain_type eq "wrfsi" && !defined $opt_m)
{
...
if( $opt_m ){
    ...
    print "Restore just the gridgen files from prior localization: move files from static_save to static \n";
...
#   }
    ...
    print "\n";
    print "     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n";
    print "     Localization is Incomplete - check the following log file:\n";
    print "     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n";
    print "\n $LAPS_DATA_ROOT/$logname/$logfilename \n";
    exit;
}
    ...
    if(-e "$LAPS_DATA_ROOT/static\_save")
    {
       ...
       if($type eq "all"){
          foreach (@filelist){
           ...
           foreach (@filelist){
#                   Skip via next for files we do not want restored from the previous localization
                    next if(/\.nl$/ || /nest7grid.parms/ || /purger.dat/ || -d "$LAPS_DATA_ROOT/static\_save/$_");
...
#                      }
    ...
    if(-e "$LAPS_DATA_ROOT/cdl\_save" && $domain_type eq "wrfsi")
    {
